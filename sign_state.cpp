...
if (courseFile.is_open() && courseFile.peek() != std::ifstream::traits_type::eof()) {
this->courseData = json::parse (courseFile);
}
courseFile.close();
}
...
void SignUpState::saveData() {
    ...
    json newData = {
            {"name", studentName},
            {"major", selectedMajor}
            {"classes", selectedClasses]}//Add Selected course data to JSON data
    };
...
void SignUpState::input(SDL_Event &event, States &state) {
    if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE) {
        ...
        if (event.type == SDL_KEYDOWN) {
            ...
            } else if (event.key.keysym.sym == SDLK_RETURN && !studentName.empty() && !selectedMajor.empty() && !selectedClasses.empty()) {
